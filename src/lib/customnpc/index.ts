...
namespace CustomNPC {
  ...
  const baseURL = "https://api.github.com/repos/ItzCandra23/CustomNPC/releases";
  ...
  export async function getAssets(url: string): Promise<GithubAsset[]|undefined> {
    try {
      ...
      return data.assets.filter((v: any) => v.name.startsWith("CustomNPC-")).map((v: any) => ({ name, filename: v.name, url: v.url, download_url: v.browser_download_url }));
    } catch(err) {
      return undefined;
    }
  }
  ...
  export async function getAllAssets(): Promise<GithubAsset[]> {
    try {
      ...
      for (const release of data) {
        const assets: GithubAsset[] = release.assets.filter(v => v.name.startsWith("CustomNPC-")).map(v => ({ url: v.url, name: release.name, tag: release.tag_name, filename: v.name, download_url: v.browser_download_url, body: release.body }));
  ...
  export async function getFile(tag: string, filename: string) {
    try {
      ...
      const response = await fetch("/customnpc/download?" + urlParams.toString());
      if (!response.ok) return undefined;
